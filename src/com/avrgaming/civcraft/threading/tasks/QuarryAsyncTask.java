...
public class QuarryAsyncTask extends CivAsyncTask {
			...
			for(ItemStack stack : contents) {
				...
				if (ItemManager.getId(stack) == CivData.WOOD_PICKAXE) {
					...
					if (rand1 < ((int)((quarry.getChance(Mineral.COAL))*randMax))) {
						newItem = ItemManager.createItemStack(CivData.COAL_ORE, 1);
					} else if (rand1 < ((int)((quarry.getChance(Mineral.COBBLESTONE))*randMax))) {
				...
				if (ItemManager.getId(stack) == CivData.GOLD_PICKAXE) {
					...
					if (rand1 < ((int)((quarry.getChance(Mineral.REDSTONE))*randMax))) {
						newItem = ItemManager.createItemStack(CivData.REDSTONE_ORE, 1);
					} else if (rand1 < ((int)((quarry.getChance(Mineral.LAPIS))*randMax))) {
						newItem = ItemManager.createItemStack(CivData.LAPIS_ORE, 1);
					} else if (rand1 < ((int)((quarry.getChance(Mineral.COBBLESTONE))*randMax))) {
						newItem = ItemManager.createItemStack(CivData.COBBLESTONE, 1);
					} else {
						newItem = getOther();
				...
				if (this.quarry.getLevel() >= 4 && ItemManager.getId(stack) == CivData.DIAMOND_PICKAXE) {
					...
					if (rand1 < ((int)((quarry.getChance(Mineral.RARE))*randMax))) {
						...
						newItem = ItemManager.createItemStack(CivData.GOLD_ORE, 1);
					} else if (rand1 < ((int)((quarry.getChance(Mineral.DIAMOND))*randMax))) {
						newItem = ItemManager.createItemStack(CivData.DIAMOND_ORE, 1);
					} else if (rand1 < ((int)((quarry.getChance(Mineral.EMERALD))*randMax))) {
	...
	private ItemStack getRare() {
		...
		int rand2 = rand.nextInt(randMax);
		if (rand2 < (randMax)) {
			return ItemManager.createItemStack(CivData.EMERALD_BLOCK, 1);
		} else if (rand2 < (randMax)) {
			return ItemManager.createItemStack(CivData.DIAMOND_BLOCK, 1);
		} else {
			return ItemManager.createItemStack(CivData.DIAMOND_BLOCK, 1);
		}
	}
